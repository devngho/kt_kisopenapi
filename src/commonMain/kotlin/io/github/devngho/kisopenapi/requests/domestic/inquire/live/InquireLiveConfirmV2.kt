...
class InquireLiveConfirmV2(override val client: KISApiClient) :
    LiveRequest<InquireLiveConfirmV2.InquireLiveConfirmData, InquireLiveConfirmV2.InquireLiveConfirmResponse> {
    ...
    private var v1Polyfill: InquireLiveConfirm? = null
    ...
    override suspend fun register(
        ...
        if (client.options.useV1PolyfillForV2 && !isATSAvailable()) {
            v1Polyfill = InquireLiveConfirm(client).also {
                v1Polyfill!!.register(
                    InquireLiveConfirm.InquireLiveConfirmData(data.corp),
                    wait,
                    force,
                    init,
                    block as suspend (InquireLiveConfirm.InquireLiveConfirmResponse) -> Unit
                )
            }
        ...
        subscribed = WebSocketSubscribed(
            this@InquireLiveConfirmV2, data, init,
    ...
    override suspend fun unregister(data: InquireLiveConfirmData, wait: Boolean) {
        if (client.options.useV1PolyfillForV2 && !isATSAvailable()) {
            v1Polyfill?.unregister(InquireLiveConfirm.InquireLiveConfirmData(data.corp), wait)
            return
        }
