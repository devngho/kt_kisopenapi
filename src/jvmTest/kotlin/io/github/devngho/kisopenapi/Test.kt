        ...
        given("API 토큰") {
            and("종목 코드") {
                ...
                `when`("InquireOverseasDetailedPrice 호출") {
                    val res =
                        InquireOverseasDetailedPrice(api)
                            .call(
                ...
                `when`("InquirePriceSeries 호출") {
                    ...
                    then("2023년 1월 1일부터 3월 31일까지의 가격을 반환한다") {
                        ...
                        res.output2!!.shouldForAll { it.bizDate!!.day shouldBeInRange 1..31 }
                ...
                `when`("InquireTodayMinute 호출") {
                    ...
                    then("30분 동안의 가격을 반환한다") {
                        val first =
                            res.output2!!
                                .minBy { it.stockConfirmTime!!.HHMMSS }
                                .stockConfirmTime!!
                                .HHMMSS
                        val last =
                            res.output2!!
                                .maxBy { it.stockConfirmTime!!.HHMMSS }
                                .stockConfirmTime!!
                                .HHMMSS
            ...
            `when`("InquireTradeVolumeRank 호출") {
                ...
                then("중복되는 순위가 없다") {
                    result.output!!
                        .map { it.rank }
                        .distinct()
                        .size shouldBe result.output!!.size
                }
                then("중복되는 종목이 없다") {
                    result.output!!
                        .map { it.ticker }
